...
mydb=mysql.connector.connect(user='root',password='Sravya1010@',host='localhost',db='snmdb')
app=Flask(__name__)
...
def updatenotes(nid):
    if session.get('user'):
        ...
        if request.method=='POST':
            ...
            cursor=mydb.cursor(buffered=True)
            cursor.execute('updatenotesdata set title=%s,description=%s where notesid=%s',[updated_title,updated_description,nid])
            mydb.commit()
            cursor.close()
...
def viewallfiles():
    if session.get('user'):
        ...
        return render_template('viewallfiles.html',fdata=stored_filedata)
    else:
        flash('pls login first') 
        return redirect(url_for('userlogin'))     
...
def getexceldata():
    if session.get('user'):
        cursor = mydb.cursor(buffered=True)
        cursor.execute('select userid from user where usermail=%s',[session.get('user')])
        user_id = cursor.fetchone() #(1,) it will be in list of tuples
...
def search():
    if session.get('user'):
        ...
        if pattern.match(usersearch):
            ...
            cursor.execute('select notesid,title,description,created_at from notesdata where notesid like %s or title like %s or description like %s or created_at like %s and added_by=%s',[usersearch+'%',usersearch+'%',usersearch+'%',usersearch+'%',userid[0]])
            resultsearch=cursor.fetchall()
            cursor.execute('select fid,filename,created_at from file_data where fid like %s or filename like %s or created_at like %s and added_by=%s',[usersearch+'%',usersearch+'%',usersearch+'%',userid[0]])
...
def fgtpwd():
    if request.method=='POST':
        ...
        count_usermail=cursor.fetchone() #if account there (1,) and if no account (0,)
        if count_usermail[0]==1:
            subject='reset link for simple notes management system'
            body=f"use the given link for password update {url_for('confirmpwd',udata=endata(data=user_email),_external=True)}"
...
def confirmpwd(udata):
    if request.method=='PUT': 
        ...
        mydb.commit()
        cursor.close()
        flash(f'new password updated successfully')
        return 'ok'
